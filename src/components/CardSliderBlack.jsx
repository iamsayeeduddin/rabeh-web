...
const CardSlider = ({ locale }) => {
  ...
  const isRTL = locale === "ar"; // Determine if the language is RTL
    ...
    <div className="relative mt-10 pb-5 md:pb-0 overflow-hidden">
      ...
      <div dir="ltr" className="flex">
        <motion.div
          className="flex"
          initial={false}
          animate={{ x: isRTL ? currentIndex * 356 : -currentIndex * 356 }} // Reverse direction for RTL
          transition={{ duration: 0.5 }}
        >
          {displayCards.map((src, index) => (
            <motion.div
              key={index}
              className="w-[340px] h-[210px] bg-[#2E334B] rounded-[16px] flex justify-center items-center p-4 m-2"
      ...
      {/* Force LTR on navigation buttons */}
      <div dir="ltr" className="flex md:justify-end justify-center mt-4">
        <button
          onClick={prevCard}
        ...
        >
          <svg
            ...
            style={{ transform: isRTL ? "rotate(180deg)" : "none" }} // Mirror the button in RTL
          >
            <path
